...
public class SnakesAndLadders {
     ...
     * solution uses an array for storing the indexes of the snakes and ladders and
    ...
    public static int snakesAndLadders(int[][] board) {
        ...
        int[] snakesAndLaddersMap = new int[length * length];
        Arrays.fill(snakesAndLaddersMap, -1);
        for (int i = length - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < length; j++) {
                if (board[i][j] != -1) {
                    ...
                    snakesAndLaddersMap[realIndex] = board[i][j];
                }
            }
        }
    ...
    private static int performBFS(int[] snakesAndLaddersMap, int targetPosition) {
        ...
        while (!positions.isEmpty()) {
            ...
            for (int i = 0; i < positionsCount; i++) {
                ...
                for (int j = 1; j <= 6; j++) {
                    ...
                    boolean isSnakeOrLadder = possiblePosition < snakesAndLaddersMap.length
                            && snakesAndLaddersMap[possiblePosition] != -1;
                    if (isSnakeOrLadder) {
                        possiblePosition = snakesAndLaddersMap[possiblePosition];
                    }
    ...
    /**
     * This solution uses a map for storing the indexes of the snakes and ladders and
     * their target values and BFS traversal until the target index is reached. Time
     * complexity is O(n^2) where n is the length of the board.
     *
     * @param board
